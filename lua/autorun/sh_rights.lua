CreateConVar( "sv_rights_active","0", FCVAR_REPLICATED, "Enables or disables the overall rights management system." )
CreateConVar( "sv_ban_spawn","0", _, "Restrict players from spawning props or entities." )
CreateConVar( "sv_ban_toolgun","0", _, "Restrict usage of the Tool Gun." )
CreateConVar( "sv_ban_physgun","0", _, "Restrict usage of the Physics Gun." )
CreateConVar( "sv_ban_gravgun","0", _, "Restrict usage of the Gravity Gun." )
CreateConVar( "sv_ban_spawnweapon","0", _, "Restrict spawning weapons from the spawnmenu." )
CreateConVar( "sv_ban_pickup","0", _, "Restrict players from picking up props (E key)." )
CreateConVar( "sv_ban_break", "0", _, "Restrict players from damaging props and entities." )
CreateConVar( "sv_ban_noclip", "0", _, "Restrict usage of noclip" )
CreateConVar( "sv_ban_cmenu","0", FCVAR_REPLICATED, "Restrict usage of the Context Menu (C key)." )
CreateConVar( "sv_rights_superadmins","1", FCVAR_REPLICATED, "Determines whether restrictions apply to superadmins (ULX)." )
CreateConVar( "sv_rights_admins","0",FCVAR_REPLICATED, "Determines whether restrictions apply to admins (ULX)." )
CreateConVar( "sv_rights_operators","0",FCVAR_REPLICATED, "Determines whether restrictions apply to operators (ULX)." )
CreateConVar( "sv_rights_maxvelocity_enabled", "0", FCVAR_REPLICATED, "Enable player movement speed limits." )
CreateConVar( "sv_rights_maxvelocity", "600", _, "Sets the maximum velocity allowed for players.", 0, 3500 )
...
hook.Add( "InitPostEntity", "Get file settings", function()
	if file.Exists( "rights_settings.txt", "DATA" ) then
		...
		for k,v in pairs( tbl ) do
			pcall( function()
				GetConVar(k):SetString( tostring(v) )
			end )
		end
	end
...
if SERVER then
	...
	hook.Add( "GravGunPunt", "BanGravGunPunt", function ( ply, _ )
				if GetBan( "sv_ban_gravgun" ) then
			return CheckRights( ply )
		end
	end )
	...
	-- Max Speed
	local DefaultMaxSpeed = 600
	local DefaultRun = 400
	local DefaultWalk = 200
	local DefaultAlt = 100
	...
	local function ApplyRightsSpeed( ply )
		...
		if is_enabled and not CheckRights( ply ) then
			local ratio = max_vel / DefaultMaxSpeed

			ply:SetMaxSpeed( max_vel )
			ply:SetRunSpeed( DefaultRun * ratio )
			ply:SetWalkSpeed( DefaultWalk * ratio )
			ply:SetSlowWalkSpeed( DefaultAlt * ratio )
		else
			ply:SetMaxSpeed( DefaultMaxSpeed )
			ply:SetRunSpeed( DefaultRun )
			ply:SetWalkSpeed( DefaultWalk )
			ply:SetSlowWalkSpeed( DefaultAlt )
		end
	end

	hook.Add( "PlayerSpawn", "Rights_SetSpeedOnSpawn", function( ply )
		timer.Simple( 0.1, function()
			ApplyRightsSpeed( ply )
		end )
	end )
	...
	cvars.AddChangeCallback( "sv_rights_admins", UpdateAllSpeeds, "Rights_SpeedUpdate_A" )
	cvars.AddChangeCallback( "sv_rights_operators", UpdateAllSpeeds, "Rights_SpeedUpdate_Op" )
